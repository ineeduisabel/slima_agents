...
uv run slima-agents plan-build "建構一個奇幻世界觀"      # 產生 TaskPlan JSON
...
└── agents/
    ...
    └── plan_builder.py       # extract_json_object + PLAN_BUILD_SYSTEM_PROMPT（plan-build 指令用）

docs/
├── frontend-integration.md  # 前端整合指南：plan-build + status + task-pipeline 串接
├── task-pipeline-guide.md   # task-pipeline JSON 格式 + context 傳遞機制
├── worldbuild-pipeline.md   # Worldbuild 完整工作流 + prompt 模板文件（已歸檔）
└── mystery-pipeline.md      # Mystery 完整工作流 + prompt 模板文件（已歸檔）
```
...
- `plan_build_result`：plan-build 完成（含 plan_json, session_id, num_turns, cost_usd, duration_s）
...
完整指南：`docs/frontend-integration.md`

**核心工作流：**
```
plan-build "需求" → TaskPlan JSON → 使用者確認/編輯 → task-pipeline < plan.json
```

**Electron 串接：**
```
Electron Main → spawn("slima-agents", ["plan-build", "--json-progress", prompt])
             → stdout readline → plan_build_result event → 顯示 plan 給使用者
